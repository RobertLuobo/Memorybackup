...
def train_ch5(net, train_iter, test_iter, batch_size, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            print("Before iteraion:%d" % iteration_num, sep="\t")
            ...
            if iteration_num > 0:
                ...
                print(id(torch.load(path_model)))
                print(id(net))
                # net = net.to(device)
                print("net in CUDA:",next(net.parameters()).is_cuda)
                # print(net_load)
                # 加载模型参数
                # state_dict_load = torch.load(path_state_dict)
                print("torch.load(%s)" % path_model)
                net = net.to(device)
            ...
            if iteration_num > 0:
                ...
                print("torch.save(net, %s)" % path_model)
                # net.to('cpu')
            ...
            print("After iteraion:%d" % iteration_num, sep="\t")
            del net
            tool.print_get_gpu_info()
